...
const LatestInspectionsTableCard = ({ title, data }) => {
  ...
  return (
    <MainCard title={title} content={false}>
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell>Référence Interventions</TableCell>
            ...
            <TableBody>
              {(data || rows).map((row, index) => (
                <TableRow hover key={index}>
                  ...
                  <TableCell>{row.chantier?.reference}</TableCell>
                  <TableCell>{row.technicien?.name}</TableCell>
                  <TableCell align="start" sx={{ pr: 3 }}>
                    {row.fin}
                  </TableCell>
                </TableRow>
      ...
      <CardActions sx={{ justifyContent: 'flex-end' }}>
        <Button variant="text" size="small" onClick={() => navigate('/inspections/list')}>
          Voir tout
        </Button>
      </CardActions>
    </MainCard>
...
LatestInspectionsTableCard.propTypes = {
  title: PropTypes.string
};

export default LatestInspectionsTableCard;
