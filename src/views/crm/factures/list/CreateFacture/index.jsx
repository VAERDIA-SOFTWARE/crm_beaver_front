...
function CreateFacture({ readOnly = false, hideTitle = false }) {
  ...
  const getCommandesQuery = useGetCommandes({ paginated: false });
  ...
  const getCommandeQuery = useGetLotChantier(selectedCommandeId || getFactureQuery.data?.d_lot_chantier_id);

  const [open, setOpen] = useState(false);
  const commandeArticlesQuery = useGetCommandeArticles({ commandeId: selectedCommandeId || getFactureQuery.data?.d_lot_chantier_id });
  ...
  async function handleSubmit(e) {
    ...
    if (isUpdate) {
      res = await updateFactureMutation.mutateAsync({
        lotChantierId: getFactureQuery.data?.d_lot_chantier_id || selectedCommandeId,
        factureId: factureId,
        body: {
  ...
  useEffect(() => {
    if (commandeArticlesQuery.isSuccess) {
      setLocalCommandesArticlesData(
        commandeArticlesQuery.data?.map((e) => {
          return {
            ...e,
            id: uuidv4()
          };
    ...
    return () => {};
  }, [commandeArticlesQuery.data, commandeArticlesQuery.isSuccess]);
    ...
    <>
      ...
      >
        ...
        <Grid container spacing={gridSpacing}>
          ...
          {!isUpdate && !isCreateAvoir && readOnly && !commandeId && getCommandesQuery?.data && getFactureQuery.data?.d_lot_chantier_id && (
            <Grid item xs={12} md={4}>
          ...
          {!isUpdate && !isCreateAvoir && !readOnly && !commandeId && getCommandesQuery?.data && (
            <Grid item xs={12} md={4}>
              <InputLabel>Référence Commande</InputLabel>
              <Autocomplete
                disabled={readOnly}
