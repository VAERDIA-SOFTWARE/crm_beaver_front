...
import { useGetJoursFeries, useGetSettingsPreferences } from 'services/settings.service';
...
const JourFerieList = () => {
  ...
  const getClientsQuery = useGetJoursFeries({ page, searchFilter });
    ...
    <MainCard
      ...
      secondary={
        <Grid item xs={12} sm={12} sx={{ textAlign: 'start' }}>
          <Tooltip title="Ajouter Jour Férié">
            <Fab
              color="primary"
              size="small"
              onClick={() => navigate(`/settings/jour-ferie/create`)}
  ...
  return (
    <div>
      <IconButton
        ...
        size="large"
        onClick={(e) => {
          navigate(`/settings/jour-ferie/${params?.row?.id}/jour-ferie-children/list`);
        }}
      >
      ...
      <IconButton
        ...
        size="large"
        onClick={(e) => {
          navigate(`/settings/jour-ferie/${params?.row?.id}/update`);
        }}
      >
...
function TableDataGrid({ setSearchFilter, getClientsQuery, setPage }) {
  ...
  const columns = [
    ...
    { field: 'nom', headerName: 'Nom', sortable: false, filterable: false, minWidth: 100, flex: 1 },
