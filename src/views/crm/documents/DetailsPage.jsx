...
import { useGetDocument } from 'services/document.service';
import { useValidateDocuments } from 'services/document.service';
...
const DocumentDetailsPage = () => {
  ...
  const chantierStatsData = getDocumentQuery.data?.chantierStatsArray;
  // const validateLotMutation = useValidateDocuments(documentId);
  // diloag state
  ...
  //     const res = await axiosClient.get(`${documentData?.pdf_download_link}`);
  //     return res.data;
  //   }
  ...
  //   });
  // }, [documentData?.pdf_download_link]);
  ...
  return (
    <MainCard
      title={`Document ${documentQueryData?.intitule ? '- ' + documentQueryData?.intitule : ''}`}
      backButton
      goBackLink="/documents"
      secondary={
        <div
          style={{
            display: 'flex',
        ...
        >
          {user?.role.includes('admin') && documentQueryData?.validated === -1 && (
            <>
              {/* <LoadingButton
                loadingPosition="end"
          ...
          {!user?.role.includes('admin') && documentQueryData?.validated === -1 && <ContentPasteRoundedIcon sx={{ color: '#ffa500' }} />}
          {documentQueryData?.validated !== -1 && (
            <div style={{ marginRight: '2rem' }}>
              {documentQueryData?.validated === 1 && <ContentPasteRoundedIcon sx={{ color: '#16a34a' }} />}
              {documentQueryData?.validated === 0 && <ContentPasteOffRoundedIcon sx={{ color: '#dc2626' }} />}
            </div>
          )}
    ...
    >
      <div>
            ...
            <Grid item xs={12}>
              <Grid container spacing={gridSpacing}>
                <Grid item xs={12} sm={12} md={12}>
                  <DocumentDataCard documentData={documentQueryData} />
                </Grid>
                <Grid item xs={12} sm={12} md={12}>
                  <iframe
                    style={{
                    ...
                    src={`${process.env.REACT_APP_API_URL}documents/${documentId}/download-pdf`}
                    width="100%"
                    height="900px"
                  ></iframe>
                </Grid>
        ...
        <TabPanel value={activeTab} index={1}>
          <DocumentVersionsList documentId={documentId} documentQueryData={documentQueryData} />
        </TabPanel>
        {false && (
        ...
        >
          <Typography variant="h5">
            Créé le {documentQueryData?.created_at && format(new Date(documentQueryData?.created_at), 'dd/LL/yyyy hh:mm:ss')}
          </Typography>
        </div>
      </div>
    </MainCard>
